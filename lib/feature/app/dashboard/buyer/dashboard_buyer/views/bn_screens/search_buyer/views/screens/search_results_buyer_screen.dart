...
class _SearchResultBuyerScreenState extends State<SearchResultBuyerScreen> {
  ...
  Widget build(BuildContext context) {
     return Scaffold(
           ...
           body: Obx(() {
             if (searchController.isNoResults.value) {
      ...
      return Padding(
             ...
             child: Obx(() =>
                 Column(
                   children: [
                     RowDividerWidget(
                       text: '${searchController.ads.length} ${context
                           .localizations.ad}',
                         ...
                         children: [
                           GridView.builder(
                             ...
                             gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                               ...
                               childAspectRatio: 160.w /
                                   281.h, // Width on Height
                             ),
                             itemBuilder: (BuildContext context, int index) {
                               return InkWell(
                                   ...
                                   child: AppAdsCarContainer(
                                     nameCar: searchController.ads[index].car
                                         ?.name ?? "no name",
                                     id: searchController.ads[index].id ?? 0,
                                     isFavorite: searchController.ads[index]
                                         .isFavorite ?? false,
                                     gallery: searchController.ads[index]
                                         .gallery,
                                     priceCar: searchController.ads[index]
                                         .price ?? "no price",
                                     conditionCar: searchController.ads[index]
                                         .mechanicalStatus?.name ?? "no status",
                                     imageSeller: searchController.ads[index]
                                         .store!.gallery![0].image ?? "",
                                     sellerName: searchController.ads[index]
                                         .store?.name ?? "no name ",
                                     nameLocation: searchController.ads[index]
                                         .address?.city?.name ?? "",
                                   ));
                             },
                           ),
